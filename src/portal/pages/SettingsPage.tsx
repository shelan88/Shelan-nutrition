...
function LanguageSection() {
  ...
  const handleChange = async (value: "en" | "ar") => {
    ...
    if (ok) {
      ...
      setToast({ type: "error", msg: "Failed to save preference." });
    }
    setTimeout(() => setToast(null), 3000);
  };

  return (
    <SectionCard title="Language Preference" icon={<Globe size={16} />}>
      ...
      {saving && <p className="text-xs text-ivory/40 mt-2">Saving…</p>}
      {toast && <Toast type={toast.type} message={toast.msg} />}
    </SectionCard>
  );
}
...
function NotificationsSection() {
  ...
  const toggles = [
    { key: "email_updates",         label: "Email updates from SHELAN"        },
    { key: "appointment_reminders", label: "Appointment reminder notifications" },
  ];
...
function DeleteAccountSection() {
  ...
  const handleDelete = async () => {
    ...
    if (!ok) {
      setToast({ type: "error", msg: "Failed to deactivate account. Please contact support." });
      setLoading(false);
    }
  ...
  return (
    <SectionCard title="Delete Account" icon={<Trash2 size={16} />}>
      <p className="text-sm text-ivory/50 mb-4">
        Deleting your account will sign you out and mark your profile as inactive.
        Your data will be retained for the nutritionist's records.
      </p>
...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <h1 className="font-heading text-2xl font-bold text-ivory">Settings</h1>
