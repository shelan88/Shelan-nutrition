...
export default function ProgramsAdminPage() {
  ...
  function cancel() { setView("list"); setEditing(null); }
  ...
  async function handleSave() {
    ...
    const payload = {
      ...
      // Clear discount_percent if discount is disabled
  ...
  async function handleReorder(id: string, direction: "up" | "down") {
    ...
    const [ok1, ok2] = await Promise.all([
      updateProgram(aRow.id, { sort_order: bOrder }),
      updateProgram(bRow.id, { sort_order: aOrder }),
    ]);
    if (!ok1 || !ok2) {
      console.error("[programs] reorder failed for", aRow.id, bRow.id);
    }
    await load();
  ...
  return (
    <div>
      <PageHeader
        title={lang === "ar" ? "البرامج" : "Programs"}
        description={lang === "ar" ? "إدارة البرامج الغذائية والصحية المقدمة للعملاء." : "Manage nutrition and wellness programs offered to clients."}
        breadcrumbs={[
      ...
      <AnimatePresence mode="wait">
          ...
          <motion.div key="list" {...fadeUp()}>
            <div className="bg-[var(--admin-surface)] rounded-2xl border border-[var(--admin-border)] overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--admin-border)]">
                ...
                >
                  <Plus size={15} />
                  {lang === "ar" ? "إضافة برنامج" : "New Program"}
                </button>
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-[var(--admin-hover-bg)]">
                      <tr>
                        <th className="text-start px-4 py-2.5 text-[11px] font-bold text-[var(--admin-text-faint)] uppercase tracking-wider whitespace-nowrap w-8">
                          {lang === "ar" ? "الترتيب" : "Order"}
                        </th>
                        <th className="text-start px-4 py-2.5 text-[11px] font-bold text-[var(--admin-text-faint)] uppercase tracking-wider whitespace-nowrap">
                          {lang === "ar" ? "الاسم" : "Name"}
                        </th>
                        <th className="text-start px-4 py-2.5 text-[11px] font-bold text-[var(--admin-text-faint)] uppercase tracking-wider whitespace-nowrap">
                          {lang === "ar" ? "السعر" : "Price"}
        ...
        /* ── EDIT / CREATE FORM ───────────────────────────────────────────── */
          <motion.div key="edit" {...fadeUp()}>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {saving
                    ...
                    : (lang === "ar" ? "إنشاء برنامج" : "Create Program")}
                </button>
              </div>
            </div>
            ...
            {/* Save error banner */}
            {saveError && (
              <div className="mt-3 flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-50 border border-red-200 text-red-600 text-[13px]">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={form.name_en} onChange={(e) => set("name_en", e.target.value)} className={inp} placeholder="Program name in English" />
                  </div>
                  <div>
                    <label className={lbl}>Name (AR)</label>
                    <input dir="rtl" value={form.name_ar ?? ""} onChange={(e) => set("name_ar", e.target.value)} className={inp} placeholder="اسم البرنامج بالعربية" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className={lbl}>Subtitle (EN)</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className={lbl}>Badge (EN) <span className="font-normal normal-case opacity-60">— optional, e.g. "Most Popular"</span></label>
                  ...
                  <div>
                    ...
                    <input dir="rtl" value={form.badge_ar ?? ""} onChange={(e) => set("badge_ar", e.target.value)} className={inp} placeholder='مثال: الأكثر شعبية' />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <textarea rows={2} value={form.short_description_en ?? ""} onChange={(e) => set("short_description_en", e.target.value)} className={`${inp} resize-y`} placeholder="Brief summary in English…" />
                  </div>
                  <div>
                    <label className={lbl}>Short Description (AR)</label>
                    <textarea dir="rtl" rows={2} value={form.short_description_ar ?? ""} onChange={(e) => set("short_description_ar", e.target.value)} className={`${inp} resize-y`} placeholder="وصف مختصر بالعربية…" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className={lbl}>Full Description (EN)</label>
                    <textarea rows={5} value={form.full_description_en ?? ""} onChange={(e) => set("full_description_en", e.target.value)} className={`${inp} resize-y`} placeholder="Full program description in English…" />
                  </div>
                  <div>
                    <label className={lbl}>Full Description (AR)</label>
                    <textarea dir="rtl" rows={5} value={form.full_description_ar ?? ""} onChange={(e) => set("full_description_ar", e.target.value)} className={`${inp} resize-y`} placeholder="وصف كامل بالعربية…" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <textarea rows={5} value={featuresEnText} onChange={(e) => setFeaturesEnText(e.target.value)} className={`${inp} resize-y`} placeholder={"Personalized meal plan\nWeekly check-ins\nProgress tracking"} />
                  </div>
                  <div>
                    <label className={lbl}>Features (AR)</label>
                    <p className="text-[11px] text-[var(--admin-text-faint)] mb-1.5">ميزة واحدة في كل سطر</p>
                    <textarea dir="rtl" rows={5} value={featuresArText} onChange={(e) => setFeaturesArText(e.target.value)} className={`${inp} resize-y`} placeholder={"خطة وجبات مخصصة\nمتابعة أسبوعية\nتتبع التقدم"} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className={lbl}>CTA Button Text (EN) <span className="font-normal normal-case opacity-60">— optional</span></label>
                    <input value={form.cta_text_en ?? ""} onChange={(e) => set("cta_text_en", e.target.value)} className={inp} placeholder='e.g. Get Started' />
                  </div>
                  <div>
                    <label className={lbl}>CTA Button Text (AR)</label>
                    <input dir="rtl" value={form.cta_text_ar ?? ""} onChange={(e) => set("cta_text_ar", e.target.value)} className={inp} placeholder='مثال: ابدئي الآن' />
                  </div>
                </div>
                ...
                <div className="border-t border-[var(--admin-border)] pt-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    ...
                    {/* Gradient */}
                    <div>
                      <label className={lbl}>Icon Gradient</label>
                    ...
                    <div>
                      ...
                      <input type="number" min={1} value={form.duration_weeks ?? 4} onChange={(e) => set("duration_weeks", Number(e.target.value))} className={inp} />
                    </div>
                ...
                <div className="border-t border-[var(--admin-border)] pt-6">
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Percent + preview — only when enabled */}
