...
export default function ConsultationsAdminPage() {
  ...
  async function toggleSection() {
    ...
    const base: Record<string, unknown> = (current && typeof current === "object" && !Array.isArray(current))
      ? { ...(current as object) } : {};
    if (!("programs" in base)) base.programs = true;
    base.consultations = next;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className={`text-[13px] font-semibold ${sectionOn ? "text-emerald-800" : "text-[var(--admin-text-muted)]"}`}>
                {sectionOn
                  ? L("Consultations section is visible on the public website", "قسم الاستشارات ظاهر على الموقع العام")
                  : L("Consultations section is hidden from the public website", "قسم الاستشارات مخفي عن الموقع العام")}
              </p>
              <p className={`text-[11px] mt-0.5 ${sectionOn ? "text-emerald-600" : "text-[var(--admin-text-faint)]"}`}>
                {L("Individual package active/inactive status is separate", "حالة كل باقة (نشطة/غير نشطة) مستقلة عن هذا الإعداد")}
              </p>
            </div>
          </div>
          <button
