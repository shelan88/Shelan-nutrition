/**
 * One-time seed: migrates static successStories + testimonials from
 * src/content/content.ts into Supabase.
 *
 * Safe to re-run: skips insert if rows already exist in each table.
 *
 * Run: node scripts/seed-stories-testimonials.mjs
 */

// Uses the Supabase REST API directly via fetch — no WebSocket issues.
...
// ─── Source data (copied verbatim from src/content/content.ts) ────────────────
...
async function run() {
  // ── Success Stories ─────────────────────────────────────────────────────────
  const existingStories = await restGet("success_stories", "select=id");
  if (existingStories > 0) {
    console.log(`✓ success_stories already has ${existingStories} row(s) — skipping insert.`);
  } else {
    const storyRows = successStoriesEn.map((en, i) => ({
      title_en:   en.title,
      title_ar:   successStoriesAr[i].title,
      story_en:   en.content,
      story_ar:   successStoriesAr[i].content,
      published:  true,
      sort_order: i + 1,
    }));
    await restInsert("success_stories", storyRows);
    console.log(`✓ Inserted ${storyRows.length} success stories.`);
  }

  // ── Testimonials ─────────────────────────────────────────────────────────────
  const existingTestimonials = await restGet("testimonials", "select=id");
  if (existingTestimonials > 0) {
    console.log(`✓ testimonials already has ${existingTestimonials} row(s) — skipping insert.`);
  } else {
    const testimonialRows = testimonialsEn.map((en, i) => ({
      client_name:    "",        // source data carries no attribution
      client_name_ar: null,
      content_en:     en,
      content_ar:     testimonialsAr[i],
      published:      true,
      rating:         5,         // all are positive endorsements
    }));
    await restInsert("testimonials", testimonialRows);
    console.log(`✓ Inserted ${testimonialRows.length} testimonials.`);
  }
